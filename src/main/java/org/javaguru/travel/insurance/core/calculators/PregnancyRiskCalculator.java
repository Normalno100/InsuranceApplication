...
/**
 * Калькулятор риска беременности (PREGNANCY)
 *
 * Покрывает: медицинские расходы связанные с беременностью до 31 недели
 *
 * ФОРМУЛА:
 ...
 * - Risk_Coefficient: коэффициент риска PREGNANCY (обычно 0.20)
 * - Age_Modifier: возрастной модификатор (растет после 30 лет)
 *
 * ВОЗРАСТНЫЕ МОДИФИКАТОРЫ:
 * - 18-30 лет: modifier = 1.0 (оптимальный возраст)
 * - 31-35 лет: modifier = 1.2 (+20%)
 * - 36-40 лет: modifier = 1.5 (+50% - поздняя беременность)
 * - 41-50 лет: modifier = 2.0 (+100% - очень поздняя беременность)
 *
 * ОГРАНИЧЕНИЯ:
 * - Только для женщин
...
public class PregnancyRiskCalculator implements RiskPremiumCalculator {
    ...
    public BigDecimal calculatePremium(TravelCalculatePremiumRequest request) {
        log.debug("Calculating PREGNANCY premium for {} {}",
        ...
        // 4. Получаем возрастной модификатор для беременности
        BigDecimal ageModifier = ageRiskPricingService.getAgeRiskModifier(
                getRiskCode(),
                age,
                request.getAgreementDateFrom()
        );
        log.debug("Age modifier for PREGNANCY: {} (age: {})", ageModifier, age);
        ...
        log.info("PREGNANCY premium: {} EUR " +
